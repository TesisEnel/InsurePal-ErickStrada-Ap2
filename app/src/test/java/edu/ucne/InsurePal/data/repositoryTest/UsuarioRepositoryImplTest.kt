...
class UsuarioRepositoryImplTest {
    ...
    fun `getUsuarios devuelve éxito con lista de usuarios`() = runTest {
        ...
        TestCase.assertEquals(2, emissions.size)
        TestCase.assertTrue(emissions[0] is Resource.Loading)
        TestCase.assertTrue(emissions[1] is Resource.Success)
        TestCase.assertEquals(domainList, (emissions[1] as Resource.Success).data)
    ...
    fun `getUsuarios devuelve error`() = runTest {
        ...
        TestCase.assertEquals(2, emissions.size)
        TestCase.assertTrue(emissions[0] is Resource.Loading)
        TestCase.assertTrue(emissions[1] is Resource.Error)
        TestCase.assertEquals(errorMessage, (emissions[1] as Resource.Error).message)
    ...
    fun `postUsuario devuelve éxito`() = runTest {
        ...
        TestCase.assertEquals(expectedDomain, (result as Resource.Success).data)
    ...
    fun `postUsuario devuelve error`() = runTest {
        ...
        TestCase.assertEquals(errorMessage, (result as Resource.Error).message)
    ...
    fun `putUsuario devuelve éxito`() = runTest {
        ...
        TestCase.assertTrue(result is Resource.Success)
        TestCase.assertEquals(Unit, (result as Resource.Success).data)
    ...
    fun `getUsuario devuelve éxito`() = runTest {
        val userId = 5
        val response =
            UsuarioResponse(usuarioId = userId, userName = "Usuario 5", password = "user5pass")
        val expectedDomain = response.toDomain()
        ...
        TestCase.assertTrue(emissions[0] is Resource.Loading)
        TestCase.assertTrue(emissions[1] is Resource.Success)
        TestCase.assertEquals(expectedDomain, (emissions[1] as Resource.Success).data)
    ...
    fun `getUsuario devuelve error por ID nulo`() = runTest {
        ...
        TestCase.assertEquals(1, emissions.size)
        TestCase.assertTrue(emissions[0] is Resource.Error)
        TestCase.assertEquals(
            "ID de usuario no puede ser nulo",
            (emissions[0] as Resource.Error).message
        )
